...
## 기초부터 시작하는 - 방송을 위한 OBS와 세팅 이야기  

오디오 인터페이스 | VST 플러그인:리버브 등등 | DAW 사용법 | 믹싱의 개론 | OBS와 연결
...
‘트랙’과 채널은 혼용해서 사용되기도 하지만 정확히는 다른 용어입니다.  
...
2) REAPER 설정 및 믹싱 개론
		 ...
		 OBS에서 설정하였듯이 ‘OBS output’으로 설정합니다. 출력을 받아들이는 것이므로 Receive 모드로, 채널 수는 스테레오이므로 ‘2’로 설정합니다. 공백문자는 쉬프트+스페이스바로 입력합니다. 
...
https://upload.wikimedia.org/wikipedia/commons/f/f2/VU_Meter.jpg
		...
		*실제 VU미터는 표시되는 값이 현재 소리의 크기가 아닌 이전 값으로부터 이동 중인 상태를 나타내므로 입력되는 신호의 세기는 0보다 더 위에 있습니다. 따라서 0 이상의 부분도 미터에 표기해두고 피크가 일어나고 있다는 것은 별도로 표기합니다. 사실 이로부터 구분할 수도 있지만, 이는 필수 사항이 아니므로 정확한 방법은 이렇게 다른 미터와 비교하여 확인하는 것입니다. 
		 ...
		 딜레이 트랙을 먼저 설정하겠습니다. 우선 FX로 ‘Tube Delay’와 ‘RED 2’를 추가합니다. 
		  ...
		  마찬가지로 프리셋을 골라서 적용하고 조금씩 조절하시면 좋습니다. softube의 플러그인들은 바로 프리셋을 적용하는 방식과 프리셋만을 위한 창을 새로 열어서 상세한 설명을 볼 수 있게 되어있습니다. 각 프리셋을 적용하기 전에 한 번씩 일어보는 것을 추천해 드립니다. 물론 Mix를 100% WET으로, 출력 초기화를 진행하신 뒤 Drive나 EQ는 조금씩 조절하시면 됩니다. 
...
6. 마치며
	 쓰다 보니 엄청나게 길어졌습니다. 원래는 아주 간단하게 한두 페이지로 끝날 줄 알았는데 어느새 20페이지 가까이 왔습니다. 그만큼 도움이 되고 싶은 바람이라 생각합니다. =)
	 + 개정판 현재 46페이지 달성... 완독하실지 내기를 건다면 도중에 때려치우신다는 것에 만 원 겁니다. 하지만 다 읽고 만 원을 받아 가시면 좋겠습니다. 그리고 앞으로 내용이 추가된다면 디스코드와 연결하는 내용 정도일 것이라 희망합니다.
	 참고로 이번에 사용한 플러그인들은 사실 OBS에서 모두 실행이 가능합니다. 다만 실험해본 결과 5번 중 3번은 OBS가 터지더랍니다. 그래서 외부 프로그램으로 돌리고 소리만 전달하는 방법을 찾게 되었습니다. 무료 DAW를 추천할까 하다가 기능 제한이나 진입 장벽이 높아서 다음에 ‘레코딩 가이드’를 작성하게 된다면 그때나 설명하고 이번에는 PedalBoard2 라는 가벼운 VST Host를 찾게 되었습니다. 
	 + 이번 개정판을 통해 실제로 다루게 되었습니다! 와! 이런. 저는 유튜브만 하지 방송은 몇 번 해보지 않았습니다. 그래서 스트리머의 입장에서 OBS는 OBS대로, PedalBoard2는 그거대로 동시 운영하려다 보니 한눈에 들어오지 않는다는 것을 미처 몰랐습니다. 동백님 지송... VST Host는 그냥 플러그인을 불러오기만 하다 보니 트랙의 개념이 없어서 레벨 미터기도 없습니다. 눈으로 MR하고 Vocal 간의 밸런스를 확인할 수 없고, 딜레이나 리버브의 정도를 조절하기 간단하지 않은 단점이 있었습니다. 따라서 DAW를 사용하는 것이 배우기에는 힘들겠지만 오래오래 방송하실 거니까 ^^7 한번 배워두면 커버곡을 스스로 믹싱할 수도 있고 본인이 원하는 소리를 만들기 더 좋을 것 같아 이렇게 가이드를 작성하게 되었습니다. 
	 + 저는 이미 큐베이스라는 DAW를 구매해서 사용 중입니다. 위에서 설명해 드렸듯이 기능 제한이 있는 저렴한 버전입니다. 그래도 비싼 것은 마찬가지라서 ‘큐베이스를 꼭 써야 한다’는 이유도 없고 해서 다른 무료 DAW를 추천해 드리게 되었습니다. 그래서 저도 새로 배웠습니다. 하지만 앞으로도 음악을 업으로 삼으실 테니 고등학교 입학하시면 입학증명서나 재직증명서 같은 것을 받아서 음학을 배우는 학생임을 증명하고 ‘프로툴즈’를 할인가로 구매하시는 것을 추천해 드립니다. 저도 큐베이스 버전 업그레이드하느라 돈만 안 발랐어도 프로툴즈 가는데 태운 돈이 많아 아까워서 못 바꾸고 있습니다. 저는 음악 관련 전공이 아니라 할인도 못 받습니다. 
	 + 처음에는 깔끔한 스튜디오 원을 사용하고자 설치하고 트랙 설정하다가 무료버전은 외부 플러그인이 불가능하다는 것을 알게 되어 좌절. Cakewalk는 인터페이스가 너무 복잡헤서 고민하다 REAPER를 사용하게 되었습니다. 대신 리퍼는 국내 사용자도 적어서 설정 하나만 해도 해외에서 찾아서 번역해야 하는 힘듦이 있습니다. 그러므로 어서 다른 DAW로 갈아타시는 것을 다시 한번 추천해 드립니다. 
	 오인페랑 윈도우 소리를 라우팅해주는 프로그램 중에 VoiceMeeter라는 것도 있습니다. 다만 인터페이스가 너무 별로에다가 설정도 복잡해서 저는 한두 번 쓰다가 지웠습니다. 때에 따라 더 많은 라우팅이 필요하...더라도 저는 안 쓸 겁니다.
	 + ReaStream을 사용하는 방식을 대체하고자 여러 방법을 찾던 도중 ASIO Link PRO라는 프로그램도 발견하였습니다. 오인페 드라이버의 기능을 확장해주는 제법 강력한 툴입니다. 루프백이 없는 오인페에 추가해줄 수도 있습니다. 다만 OBS와 REAPER를 동시에 돌리다 보니 약간 불안정하고 충돌하는 부분이 있었습니다. 따라서 하루 정도 테스트해보고 결국 ReaStream을 사용하는 최소화하는 방식을 찾게 되었습니다. 만약 또다른 프로그램이  ASIO 드라이버를 지원하지 않을 때는 이 프로그램으로 해결하는 것이 가장 빠를 것 같습니다. 