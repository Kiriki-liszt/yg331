...
마지막으로 panning을 주어 스테레로 차이를 만들어 준 결과 enhance에는 채널 간 영향이 존재하여 Harminics에서 단순히 gain 차이에 의해 발생할 수 없는 harmonic pattern이 발견되었고, Dynamics 분석을 통해 채널 간 간섭의 경우 어택이 존재함을 알 수 있다.  
